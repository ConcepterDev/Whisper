...
public func Whisper(message: WMessage, to: UINavigationController, action: Action = .Show) {
...
class WhisperFactory: NSObject {
  ...
  func craft(message: WMessage, navigationController: UINavigationController, action: Action) {
  ...
  func changeView(message: WMessage, action: Action) {
    presentTimer.invalidate()
    delayTimer.invalidate()
    hideView()
  ...
  func presentFired(timer: NSTimer) {
    ...
    let message = WMessage(title: title, textColor: textColor, backgroundColor: backgroundColor, images: images)
